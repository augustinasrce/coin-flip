    ...
    <>
      ...
      <div className="img-container">
        <input
          type="image"
          onClick={handleFlip}
          src={imgConfig[coinSide].url}
          alt={imgConfig[coinSide].alt}
        />
      </div>
      <div className="text-container">
        ...
        {flipCount ? (
          <p className="small">
            Heads {headsCount} ({((headsCount / flipCount) * 100).toFixed(2)}%),
            Tails {tailsCount} ({((tailsCount / flipCount) * 100).toFixed(2)}%)
          </p>
        ) : null}
      </div>
    </>
